...
namespace BLL.Services
{
        ...
        public class RayonsService : ServiceBase, IRayonsService
        {
            ...
            public ServiceBase Update(Rayons rayons)
            {
                if (_db.Rayons.Any(r => r.Id != rayons.Id && r.Name.ToUpper() == r.Name.ToUpper().Trim()))
                    return Error("This rayon is existing in the system!");
                var entity = _db.Rayons.Find(rayons.Id);
                entity.Name = rayons.Name?.Trim();
                entity.VoterNumber = rayons.VoterNumber;
                entity.OblastId = rayons.OblastId;
